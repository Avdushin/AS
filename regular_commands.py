...
async def my_bot(callback_query: types.CallbackQuery):
    ...
    if DataBase.get_data(id=message.chat.id) != None:
        ...
        for i in range(len(rez)):
            button = InlineKeyboardButton(text=str(rez[i][0]), callback_data=f'{rez[i][4]}')
...
async def bot_info_(callback_query: types.CallbackQuery):
    ...
    bot_num= callback_query.data
    if bot_data:
        chat_name, user_id, chat_id, subscription_start, bot_num = bot_data[int(bot_num)-1]
        await bot.edit_message_text(
            chat_id=message.chat.id,
            message_id=message.message_id,
            text=f'''Информация о боте №{chat_name}.:\nID пользователя: {user_id}.\nChat ID: {chat_id}.\nДата подписки: {(datetime.strptime(subscription_start, r'%Y-%m-%d') - timedelta(days=30)).strftime(r'%Y-%m-%d')}.\nДата окончания подписки: {subscription_start}.''', reply_markup=keyboard.back_keyboard)
...
async def create_bot(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(callback_query.from_user.id, 
                           """1. Добавьте этого бота в чат @nospamdotrobot. \n2. Выдайте ему всевозможные права влючая права на анонимность.\n3. Перешлите сообщения от имени чата где находится бот.""", 
                           reply_markup=keyboard.url_keyboard3)

async def get_chat_id(message: types.Message, state: FSMContext):
    try:
        async with state.proxy() as data:
            data['get_chat_id'] = message.forward_from_chat.id
            await FSM.next()
            await bot.send_message(chat_id=message.chat.id, 
                                   text="Согласны вы с правилами использования.", 
                                   reply_markup=keyboard.choice_keyboard)
    except:
        await  bot.send_message(chat_id=message.chat.id, 
                                text="Попробуйте ещё раз но но верно.", 
                                reply_markup=keyboard.choice_keyboard1)

async def chat_name(callback_query: types.CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
        if callback_query.data == "yes":
            await FSM.next()
            await bot.send_message(callback_query.from_user.id, "Напишите назавание чата что бы удобнеее ориентироваться.")
        else:
            await bot.send_message(callback_query.message.chat.id, "Отказ был принят")
            await state.finish()

async def payment_data(message : types.Message, state: FSMContext ):
    async with state.proxy() as data:
        ...
        await bot.send_invoice(chat_id=message.chat.id,
                                title="Подписка анти спам.",
                                description=f"Оплата подписки пользователь {message.chat.id}",
                                provider_token= pay_token,
                                currency="rub",
                                is_flexible=False,
                                prices=[{
                                ...
                                start_parameter="donation_to_charity",
                                payload=f"{message.chat.id, data['get_chat_id'], data['chat_name'], data['chat_name']}")
        await state.finish()
...
async def successful_payment(message: types.Message):
    ...
    if len(a) == 4:
        DataBase.insert_data(data = [a[0], a[1], (datetime.now() + timedelta(days=30)).strftime(r'%Y-%m-%d'), a[2]])
    elif len(a) == 3:
    ...
    await bot.send_message(chat_id=message.chat.id, text="Оплата прошла успешно! добавте его в группу  @nospam_no_bot и назначте администратором")
...
def commands(dp: Dispatcher, ):
    ...
    dp.register_callback_query_handler(chat_name, state=FSM.chat_name)
    dp.register_message_handler(payment_data,  state=FSM.payment_data)
    dp.register_callback_query_handler(bot_info_)